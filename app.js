...
function branchState({angle, branchWidth}) {
	this.angle = angle
	this.branchWidth = branchWidth
}

function makeTree(getNumBranches, configureBranch, endCondition, makeOffBranches) {
	let numBranches = getNumBranches()
	configureBranch()
	if (endCondition()) {
		return
	}
	makeOffBranches(numBranches)
}

function thisTree(line, treeParams, branchState) {
	function getNumBranches() {
		return intrdm(treeParams.numBranchesRange)
	}
	function configureBranch() {
		var lineSize = (branchState.branchWidth*(5/8 - (branchState.numSiblings-2)/8)) * (branchState.iteration == treeParams.numRecursions ? 24 : 1)
	...
	function endCondition() {
		return branchState.iteration < treeParams.numRecursions
	}
	function makeOffBranches() {
